...
if [[ "$HOSTNAME" == "elitebook" ]]; then
    ...
    alias ms="docker run --rm --env UNISONLOCALHOSTNAME=$HOSTNAME --volume /etc/timezone:/etc/timezone:ro --volume /etc/localtime:/etc/localtime:ro --volume ~/.unison/$HOSTNAME.prf:/home/unison/.unison/$HOSTNAME.prf --volume ~/.unison:/home/unison/.unison --volume ~/.ssh:/home/unison/.ssh --volume /srv/data/my/files/code:/srv/data/my/files/code --volume /tmp/unison_sync_$HOSTNAME.log:/tmp/unison_sync_$HOSTNAME.log --interactive --tty jakepricedev/docker-unison:2.52.1-r0 $HOSTNAME"
    alias msb="docker run --rm --env UNISONLOCALHOSTNAME=$HOSTNAME --volume /etc/timezone:/etc/timezone:ro --volume /etc/localtime:/etc/localtime:ro --volume ~/.unison/$HOSTNAME.prf:/home/unison/.unison/$HOSTNAME.prf --volume ~/.unison:/home/unison/.unison --volume ~/.ssh:/home/unison/.ssh --volume /srv/data/my/files/code:/srv/data/my/files/code --volume /tmp/unison_sync_$HOSTNAME.log:/tmp/unison_sync_$HOSTNAME.log --interactive --tty jakepricedev/docker-unison:2.52.1-r0 -batch $HOSTNAME"
fi
